...
function setupEventListeners() {
    ...
    if (channelSelectCloseButton) {
        channelSelectCloseButton.addEventListener('click', () => {
            channelSelectModal.style.display = 'none';
        });
    }
    // 기간 선택
    const dateRange = document.querySelector('.date-range-controls');
    if (dateRange) {
...
async function updateUI() {
    ...
    if (channels.length > 0) {
        ...
        if (mutantVideoList) mutantVideoList.innerHTML = '<p>채널을 추가하여 영상을 분석해주세요.</p>';
        if (latestVideoList) latestVideoList.innerHTML = '<p>채널을 추가하여 영상을 분석해주세요.</p>';
    }
}
...
async function handleAddChannel() {
    ...
    try {
        ...
        do {
            ...
            if (allItems.length >= 50) break; // 한 번에 50개까지만
        } while (pageToken);
        ...
        if (allItems.length > 1) {
            ...
            addChannel(allItems[0].id.channelId);
        }
    } catch (error) {
...
function displaySearchResultsPaginated() {
    if (!channelSearchResults) return;
    const perPage = 5;
    const totalPages = Math.ceil(currentSearchResults.length / perPage);
    const pageResults = currentSearchResults.slice((currentSearchPage - 1) * perPage, currentSearchPage * perPage);
    ...
    if (paginationContainer) {
        ...
        // Page numbers (1~3까지만)
        const start = Math.max(1, currentSearchPage - 1);
...
function displayChannelList() {
    ...
    if (channelCountSpan) channelCountSpan.textContent = channels.length;
...
// 동영상 상세 정보 가져오기 (썸네일 fallback 포함)
